...
class Block:
    def __init__(self, previous_hash, difficulty, miner, miner_rewards):
        ...
        self.hash = ''  # 本區塊的哈希值
        self.difficulty = difficulty  # 困難度
        self.nonce = 0
...
class BlockChain:
    ...
    def create_genesis_block(self):  # 創造創世塊
        print("Create genesis block...")
        new_block = Block('Hello World!', self.difficulty,
                          'sky', self.miner_rewards)
    ...
    def add_transaction_to_block(self, block):
        ...
        self.pending_transactions.sort(
            key=lambda x: x.fee, reverse=True)  # 按照手續費由大排到小
    ...
    def adjust_difficulty(self):
        if len(self.chain) % self.adjust_difficulty_blocks != 1:
            ...
            average_time_consumed = round(
                (finish - start) / (self.adjust_difficulty_blocks), 2)  # 取平均後四捨五入至兩位小數位數
    ...
    def generate_address(self):  # 產生公私鑰(私鑰加密、公鑰解密)
